...
public:
	...
	void up_score(int num){
		set_score(this->_score + num);
	}
...
void chess_print(DinoPark &dinoPark, int height, int width){
...
int main() {
	...
	// dinoPark.addTree(1, 2);
	// dinoPark.addTree(1, 3);
	// dinoPark.addTree(1, 4);
	// dinoPark.addTree(5, 4);

	// ГОРИЗОНТАЛЬНАЯ ЛИНЕЙКА ЕЛОЧЕК
	// for (int i = 0; i < width; i ++){
	// 	dinoPark.addTree(i, 6);
	// }

	// ВЕРТИКАЛЬНАЯ ЛИНЕЙКА СНЕГОВИКОВ
	// for (int i = 0; i < height; i++) {
	// 	dinoPark.addSnowMan(6, i);
	// }

	// CellSize = 80;

	chess_print(dinoPark, height, width);
